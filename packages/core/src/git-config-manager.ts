...
export class GitConfigManager {
  ...
  static async applyProfile(profile: Profile): Promise<void> {
    ...
    if (profile.git.userName) {
      gitConfig.push(`[user]`);
      gitConfig.push(`\tname = ${profile.git.userName}`);
    }
    ...
    if (profile.git.userEmail) {
      ...
      gitConfig.push(`\temail = ${profile.git.userEmail}`);
    }

    const configContent = `# Auto-generated by Kontext for profile: ${profile.name}\n${gitConfig.join('\n')}\n`;
    
    try {
      await fs.promises.writeFile(this.KONTEXT_GIT_CONFIG_PATH, configContent, 'utf8');
    } catch (error) {
      throw new Error(`Failed to write Kontext git config: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
