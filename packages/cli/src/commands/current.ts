...
export const currentCommand = new Command('current')
  ...
  .action(async options => {
    try {
      ...
      if (!activeProfile) {
        console.log(error('No active profile found'));
        return;
      }
      ...
      if (options.edit) {
        ...
        try {
          ...
          console.log(
            error(`Failed to open editor. You can edit the file manually at: ${profilePath}`)
          );
        }
        return;
      }
      ...
      if (options.detailed) {
        ...
        try {
          ...
          console.log(
            error(
              `Failed to load profile details: ${err instanceof Error ? err.message : 'Unknown error'}`
            )
          );
        }
      }
      ...
      console.error(
        error(
          `Failed to get current profile: ${err instanceof Error ? err.message : 'Unknown error'}`
        )
      );
      process.exit(1);
    }
