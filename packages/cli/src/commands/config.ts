...
import { error, info, header, path, divider, command } from '../utils/prompt-utils';
  ...
  .action(async () => {
    try {
      ...
      if (fs.existsSync(profilesPath)) {
        ...
        console.log(
          '  ⚠️  Directory does not exist yet (will be created when you create your first profile)'
        );
      }
      console.log('');
      ...
      if (activeProfile && profileFilePath) {
        ...
        console.log(
          `  Context directory: ${path(DirectoryScanner.getProfileFileDirectory(profileFilePath))}`
        );
      } else {
      ...
      console.log(
        `    ${command('echo "work" > .kontext-profile')}  # Associate current directory with "work" profile`
      );
      console.log(
        `    ${command('kontext current')}                 # Check current profile status`
      );
      console.log('');
      ...
      console.log(
        `  Shell integration: Added to your shell config (${path('~/.zshrc')}, ${path('~/.bashrc')}, etc.)`
      );
      console.log('');
      ...
      console.error(
        error(
          `Failed to show configuration: ${err instanceof Error ? err.message : 'Unknown error'}`
        )
      );
      process.exit(1);
    }
