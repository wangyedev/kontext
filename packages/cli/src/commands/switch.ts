...
import { error, warning, profile as profileFormat } from '../utils/prompt-utils';
    ...
    try {
      ...
      if (GitConfigManager.isGitAvailable()) {
        try {
          ...
          console.log(
            warning(
              `Failed to apply Git configuration: ${err instanceof Error ? err.message : 'Unknown error'}`
            )
          );
        }
      }
      ...
      console.error(
        error(`Failed to switch profile: ${err instanceof Error ? err.message : 'Unknown error'}`)
      );
      process.exit(1);
    }
  });
