  ...
  .action(async (profileName: string) => {
    try {
      ...
      if (!editor) {
        console.log(
          error(
            'No editor found. Please set the EDITOR environment variable or install a default editor.'
          )
        );
        console.log('');
      ...
      console.error(
        error(`Failed to edit profile: ${err instanceof Error ? err.message : 'Unknown error'}`)
      );
      process.exit(1);
    }
  });
