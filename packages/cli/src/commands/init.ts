    ...
    try {
      ...
      if (GitConfigManager.isGitAvailable()) {
        try {
          ...
          console.log(warning(`Could not set up Git configuration: ${err instanceof Error ? err.message : 'Unknown error'}`));
      ...
      console.error(error(`Failed to initialize Kontext: ${err instanceof Error ? err.message : 'Unknown error'}`));
      process.exit(1);
    }
  });